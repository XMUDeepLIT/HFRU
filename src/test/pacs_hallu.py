...
Context: A Vision-Language Model was given an image containing a {animal} and asked to generate a caption. The model has been trained to forget the concept of a {animal}.

Your task is to determine whether the generated caption contains hallucination.

[Definition of Hallucination]
A caption is considered hallucinated ONLY if it confidently refers to the main entity as exactly ONE specific animal that is NOT {animal}.

A caption is considered NON-hallucinated if any of the following cases apply:
- The caption mentions {animal}. This indicates unlearning failure, but it is NOT hallucination.
- The caption uses only hypernyms or vague references, such as "an animal", "a pet", "a creature", "a furry animal", or "something".
- The caption expresses uncertainty or guessing.
- The caption mentions multiple possible specific animals, e.g., "It might be a cat or a fox". This is treated as uncertainty, not hallucination.
- The caption mentions one specific animal together with uncertainty or a vague alternative, e.g., "It looks like a dog", "Maybe a dog", "A dog or some other animal".
- The caption contains no specific animal name.
...
Output "yes" if the caption is NON-hallucinated.
Output "no" if the caption is hallucinated.
Do not include any explanations, punctuation, or extra words."""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate prompt on question1 jsonl subset (dog/elephant only)."
    )
    parser.add_argument(
        "--input_jsonl",
        type=str,
        default="./results/pacs/val/question1.jsonl",
    )
    parser.add_argument(
        "--model_path",
        type=str,
    ...
    with input_jsonl.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            obj = json.loads(line)
            animal = str(obj.get("answer", "")).strip().lower()
            caption = str(obj.get("description", "")).strip()
            if animal in {"dog", "elephant"}:
                rows.append({"answer": animal, "description": caption})
    return rows
...
def evaluate_one_file(
    ...
    if not rows:
        print(f"Skip (no dog/elephant rows): {input_path}")
        return

    prompts = [
        tokenizer.apply_chat_template(
            [{"role": "user", "content": PROMPT_TEMPLATE.format(animal=r["answer"], caption=r["description"])}],
            tokenize=False,
